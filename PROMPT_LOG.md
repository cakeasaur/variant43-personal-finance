...
12. [Миграция Kivy → Flet: новые экраны, UX, исправление бага шифрования](#12-миграция-kivy--flet-новые-экраны-ux-исправление-бага-шифрования)
13. [Обновление документации и финальная зачистка репозитория](#13-обновление-документации-и-финальная-зачистка-репозитория)
...
## 12. Миграция Kivy → Flet: новые экраны, UX, исправление бага шифрования

**Инструмент:** Claude Code (CLI, `claude-sonnet-4-6`)  
**Когда:** 16–17.05.2026  
**Затронуто файлов:** 41 изменён/создан

### Контекст

После базовой миграции UI на Flet 0.85 оставались нерешённые задачи:
- нестабильное шифрование при закрытии окна
- экраны Категорий и Настроек — заглушки «в разработке»
- главный экран не соответствовал макету

### 12.1 Исправление бага шифрования (atexit LIFO)

**Запрос:**
```
аудит сделанного проведи
```

**Найденная проблема:**  
`atexit` регистрирует хендлеры в порядке вызова, выполняет в обратном (LIFO).
`_new_temp_db()` регистрировал `path.unlink` позже, чем `shutdown_encrypt` в `main()`.
При выходе temp-файл БД удалялся первым — `shutdown_encrypt` находил `runtime_db.exists() == False`
и пропускал шифрование. Данные терялись.

**Решение:** убрать `atexit` из `_new_temp_db`. Очистку делает `shutdown_encrypt`
после шифрования; осиротевшие файлы подчищает `_cleanup_orphaned_dbs` при следующем запуске.

**Что отклонил:**  
Claude предлагал также переупорядочить регистрации — отклонил как усложнение;
достаточно убрать дублирующий хендлер.

### 12.2 Экран Категорий

**Запрос:**
```
давай начнем всё таки с экранов категорий и настроек
```

Реализован полный CRUD через `CategoryRepository`:
- список с бейджами типа (Доход / Расход / Оба)
- диалог создания/редактирования с `Dropdown`
- удаление (ON DELETE SET NULL сохраняет историю операций)

**Принято без изменений** — паттерн взят из `goals.py`.

### 12.3 Экран Настроек

Реализованы три секции:
- **Безопасность**: статус шифрования + смена пароля (диалог с верификацией текущего)
- **О приложении**: версия, стек
- **Разработчик**: `@cakeasaur`

Смена пароля реализована через callback `change_password(old, new) → str | None` —
проверяет текущий пароль через `ctx["passphrase"]`, обновляет in-memory.
Новый пароль применяется при следующем закрытии окна (`shutdown_encrypt`).

### 12.4 Переработка главного экрана

**Запрос:**
```
сможешь сделать главный экран как на макете?
```
...
| Элемент | Было | Стало |
|---|---|---|
| Хедер | месяц | диапазон «1–31 мая 2026» + иконка фильтра |
| 4-я карточка | Операций за месяц | До конца месяца: N дней |
| Дельты | — | ↑/↓ % vs прошлый месяц |
| Правая колонка | только цели | топ-3 расходов по категориям + цели |
| Лейаут | chart + goals рядом | chart + recent слева, категории + goals справа |

**Что скорректировал:**  
Claude предложил `ft.alignment.center` — не существует в Flet 0.85,
заменил на `ft.Alignment(0, 0)`.

### 12.5 UX-правки

- `empty_state`: добавлен `expand=True` + `alignment=ft.Alignment(0, 0)` —
  пустые состояния теперь по центру карточки на всех экранах.
- `tx_row`: операции без категории — название по центру вертикально,
  прочерк убран; `spacing=0` на Row для контролируемых отступов.

### Результат

| Метрика | До | После |
|---|---|---|
| Экраны-заглушки | 2 (Категории, Настройки) | 0 |
| Тестов | 40 | 40 (2 skipped — flet не установлен в headless) |
| Баг шифрования | воспроизводился | исправлен |

---

## 13. Обновление документации и финальная зачистка репозитория

**Инструмент:** Claude Code (CLI, `claude-sonnet-4-6`)  
**Когда:** 17.05.2026  
**Затронуто файлов:** 22 документа + 21 файл удалён

### 13.1 Обновление markdown-файлов

**Запрос:**
```
Обновляем все маркдавн файлы под новый фреймворк
```

Обновлено 14 файлов: все главы пояснительной записки, README,
DEPLOYMENT.md, ANDROID.md, диаграммы. Kivy-упоминания заменены на Flet
во всех документах кроме MIGRATION_KIVY_TO_FLET.md (исторический лог).

Добавлена таблица библиотек в `06_technology.md`, обновлена структура
проекта в `04_architecture.md` и README под реальную `src/ui_flet/`.

### 13.2 Удаление Kivy-кода
...
Удалены: `src/ui/` (15 файлов), `src/app.py`, `main.py`, `buildozer.spec`.
Обновлены: `scripts/build_win.ps1` (PyInstaller → flet build),
`.github/workflows/ci.yml` (убраны Kivy-комментарии),
`.gitignore` (Kivy-артефакты → flet build artifacts).
Добавлена запись v0.2.0 в `CHANGELOG.md`.

**Что принято без изменений:** предложенная структура трёх коммитов
(feat/docs/chore) — логичное разделение.

### Итоги использования AI (сессии 12–13)

| Задача | Польза AI |
|---|---|
| Диагностика atexit-бага | Выявил LIFO-порядок как причину; предложил минимальный фикс |
| Экраны Categories/Settings | Сгенерировал по паттерну goals.py; callback для смены пароля |
| Редизайн overview | Разбил на левую/правую колонку; добавил delta vs прошлый месяц |
| Поиск Kivy-зависимостей | Проверил build_win.ps1, ci.yml, diagrams перед удалением |

| Проблема | Что исправил |
|---|---|
| `ft.alignment.center` не существует | Заменил на `ft.Alignment(0, 0)` |
| Хотел удалить Kivy сразу | Остановил — сначала обновить build-скрипты |
